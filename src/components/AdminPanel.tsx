    ...
    <div className="fixed inset-0 bg-zinc-950 z-50 overflow-y-auto p-4 md:p-8 flex flex-col items-center justify-center">
      {!isAuthenticated ? (
        ...
        <>
                        ...
                        <div className="flex gap-2">
                          ...
                          {order.status !== 'entregue' && (
                            <button 
                              ...
                              className="flex-1 py-2 bg-green-400 border-2 border-black rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-green-500 text-black"
                            >
          ...
          <div id="printable-command" className="hidden bg-white text-black p-4">
            {orderToPrint && (
              <div className="text-sm">
                <div className="text-center mb-4">
                  <h2 className="font-bold text-lg">NICKEL LANCHES</h2>
                  <p>O Lanche Mais Divertido!</p>
                  <p>------------------------</p>
                </div>
                <div className="mb-4">
                ...
                <div className="border-t border-b border-dashed border-black py-2 mb-4">
                  {orderToPrint.items.map((item, i) => (
                    <div key={i} className="mb-2">
                      ...
                      {item.extras && item.extras.length > 0 && (
                        <div className="text-xs pl-4">+ {item.extras.map((e:any)=>e.name).join(', ')}</div>
                      )}
                      {item.observation && (
                        <div className="text-xs pl-4 italic uppercase text-gray-700">OBS: {item.observation}</div>
                      )}
                    </div>
                  ))}
                </div>
